...
- **사용자 계정**: `users` 테이블(이메일/이름/비밀번호) 전체를 조회·검색하는
  읽기 전용 화면. "이름 표시" 항목의 `MAIL_MONITOR_DB_DSN` 설정이 안 돼 있으면
  비활성화 안내만 뜬다. ⚠️ 이 테이블의 비밀번호는 평문으로 저장돼 있다 —
  화면에는 기본적으로 마스킹돼 있고, 눈 아이콘을 눌러야 그 행 하나만 보인다
  (전체를 한 번에 노출하지 않음). 이 페이지 접근 권한 관리는 전적으로
  네트워크/방화벽 수준에 달려 있으니(대시보드 자체엔 로그인이 없다)
  누가 접근 가능한 네트워크에서 이 기능을 쓸지 신중히 판단할 것.
- **포워딩 관리**: `forwardings` 테이블(source, destination) 조회·추가·삭제.
  ...
  INSERT/DELETE 권한이 있어야 추가/삭제가 동작한다.
