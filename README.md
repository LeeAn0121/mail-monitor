...
잘리지 않고 줄바꿈됨). FWD(전달)는 원본수신 컬럼에 어떤 alias로 왔는지 따로 나온다.
검색어·필터는 브라우저에 24시간 저장돼서 새로고침해도 유지되고, 우측 상단 새로고침
버튼으로 수동 재조회할 수 있다. "내보내기"로 현재 필터링된 결과를 CSV/Excel(XLSX)로
저장하거나 바로 인쇄할 수 있다. 행을 클릭하면 원본 로그 줄을 포함한 상세 팝업이 뜬다.
SENT/RECV/FWD의 처리결과엔 postfix가 남긴 실제 상태값(예: `250 2.0.0 Ok: queued as ...`)이
그대로 나온다.
