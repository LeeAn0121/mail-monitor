...
- **실시간**: 라이브 로그 피드. 이벤트 타입 칩으로 필터링, 텍스트로 발신/수신/제목/처리결과 검색.
  발신·수신 랭킹은 오른쪽에 상시 표시(각각 상위 50건, 내부 스크롤).
- **이력 검색**: 메모리 버퍼가 아니라 디스크의 `mail.log`+로테이션 파일 전체를 스캔
  (TUI의 `/` 검색과 동일 엔진). 언제·누가·누구에게·어떤 제목으로 주고받았는지 검색.

두 화면 모두 시간/유형/발신/수신/내용(제목)/처리결과/발신자IP/수신자IP 8개 컬럼을 보여준다.
...
`6`으로 켜고 끌 수 있고, 목록에서는 `↪` 글리프로 구분된다. 웹 대시보드의 처리결과
컬럼엔 "원래 주소로 발송됨 → 최종 주소로 전달" 형태로 어떤 alias가 누구에게
최종 전달됐는지 그대로 나온다.
