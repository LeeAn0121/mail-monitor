...
`mail-monitor_linux_<arch>.deb`를 받아 `sudo dpkg -i`로 설치.
...
- **실시간**: 라이브 로그 피드. 이벤트 타입 칩으로 필터링, 텍스트로 발신/수신/제목/처리결과 검색.
- **발신/수신 랭킹**: 별도 화면으로 분리 — 실시간 로그 화면은 로그 자체에만 집중.
  각각 상위 50건, 내부 스크롤.
- **이력 검색**: 메모리 버퍼가 아니라 디스크의 `mail.log`+로테이션 파일 전체를 스캔
  (TUI의 `/` 검색과 동일 엔진). 언제·누가·누구에게·어떤 제목으로 주고받았는지 검색.
...
- **사용자 계정**: `users` 테이블(이메일/이름/비밀번호) 조회·등록·수정·삭제.
  "이름 표시" 항목의 `MAIL_MONITOR_DB_DSN` 설정이 안 돼 있으면 비활성화
  안내만 뜬다. 이메일은 등록 시에만 입력하고 이후엔 고정(수정 불가) — 다른
  테이블(포워딩 등)이 전부 이메일로 계정을 참조하므로, 주소를 바꾸려면
  삭제 후 새로 등록해야 한다. ⚠️ 이 테이블의 비밀번호는 평문으로 저장돼 있다
  — 목록엔 기본적으로 마스킹돼 있고 눈 아이콘을 눌러야 그 행 하나만 보이지만,
  등록/수정 폼에는 비밀번호를 평문으로 입력·표시한다. 이 페이지 접근 권한
  관리는 전적으로 네트워크/방화벽 수준에 달려 있으니(대시보드 자체엔
  로그인이 없다) 누가 접근 가능한 네트워크에서 이 기능을 쓸지 신중히 판단할 것.
- **포워딩 관리**: `forwardings` 테이블(source, destination) 조회·추가·수정·삭제.
  양쪽 주소를 `users` 테이블과 대조해서 이름이 있으면 같이 보여준다(전달
  주소가 외부 메일이거나 `users`에 없는 주소여도 정상 표시). 이것도
  `MAIL_MONITOR_DB_DSN` 필요 — DB 접속 계정에 `forwardings` 테이블
  INSERT/UPDATE/DELETE 권한이 있어야 추가/수정/삭제가 동작한다.

왼쪽 메뉴는 "모니터링"(실시간/랭킹/이력 검색)과 "관리"(발신자 차단/사용자
계정/포워딩 관리) 두 그룹으로 나뉘어 있다. 넓은 화면에서는 그룹 라벨과
메뉴 이름이 같이 보이고, 좁은 화면(모바일)에서는 기존처럼 아이콘만 남고
누르면 툴팁으로 이름이 뜬다.

실시간/이력 화면 모두 각 행 마지막에 차단(⊘) 버튼이 있어서 화면 전환 없이
바로 그 자리에서 발신자를 차단할 수 있고, 행을 클릭해서 여는 상세 팝업에도
같은 버튼이 있다. 둘 다 클릭하면 확인 후 "발신자 차단" 화면과 동일한
API로 즉시 차단된다.

실시간/이력 화면 모두 시간/유형/발신/수신/원본수신(별칭)/내용(제목)/처리결과/발신자IP/
수신자IP 9개 컬럼을 보여준다 (MUI X DataGrid — 컬럼 너비 드래그로 조절 가능, 긴 텍스트는
잘리지 않고 줄바꿈됨). 발신/수신엔 `users` 테이블에 등록된 이름이 있으면
`email (이름)` 형태로 같이 나온다 (설정법은 "이름 표시" 항목 참고). FWD(전달)는
원본수신 컬럼에 어떤 alias로 왔는지 따로 나온다.

처리결과 컬럼엔 "발송 완료"/"수신 완료"/"반송"/"거부" 같은 짧은 요약만 보이고, postfix가
실제로 남긴 상태값(예: `250 2.0.0 Ok: queued as ...`, bounce/reject 사유 전문)은 행을
클릭하면 뜨는 상세 팝업에 "처리결과 원본"으로 전체가 나온다. 팝업엔 원본 로그 줄도 그대로
포함되고 복사 버튼도 있다.

검색어·필터는 브라우저에 24시간 저장돼서 새로고침해도 유지되고, 우측 상단 새로고침
버튼으로 수동 재조회할 수 있다. "내보내기"로 현재 필터링된 결과를 CSV/Excel(XLSX)로
저장하거나 바로 인쇄할 수 있다.

헤더의 "새로고침" 드롭다운으로 대시보드가 백그라운드에서 자동으로 다시 조회하는
주기를 10초/1분/3분/5분/15분/30분 중에서 고를 수 있다 (기본 10초). 실시간 로그
자체는 이 주기와 무관하게 SSE로 즉시 들어온다 — 이 설정은 발신/수신 랭킹과
BOUNCE/REJECT 급증 배지처럼 스냅샷을 다시 받아와야 갱신되는 정보의 주기만 조절한다.
선택한 값은 브라우저에 계속 저장된다.
...
다른 서비스와 포트가 겹치면(예: headscale도 기본 8080을 씀) 서비스용 기본 포트 자체를
바꿔야 한다 — `/etc/mail-monitor/mail-monitor.env`에 `MAIL_MONITOR_WEB_ADDR=:포트`를 쓰고
`sudo systemctl restart mail-monitor`.
...
`mail.log.1`, `mail.log.2.gz`, logrotate `dateext` 설정을 쓰는 서버의
`mail.log-20260101.gz` 같은 파일까지 뒤지려면 `/` (웹 대시보드의 "이력 검색"도
동일). 어떤 로테이션 명명 방식이든 `mail.log`와 같은 디렉터리에서 파일명이
`mail.log` 뒤에 `.`나 `-`로 이어지는 파일을 전부 찾아 수정시각 순으로 스캔한다
— `.gz`면 gzip도 자동으로 풀어서 읽는다. Queue-ID로 발신/수신 상관관계를
다시 매칭한 다음 검색어와 맞는 것만 보여준다. `mail.log`를 직접 읽을 권한이
없으면 `sudo tail`로 자동 대체한다 (라이브 tail이 이미 하던 것과 동일한
fallback — 압축 파일도 바이트 그대로 받아서 이쪽에서 gzip 해제). `esc`로
실시간 화면으로 복귀.
...
`6`으로 켜고 끌 수 있고, 목록에서는 `↪` 글리프로 구분된다. 웹 대시보드에선 원본수신
컬럼에 원래 주소(alias)가, 수신 컬럼에 실제 전달된 주소가 따로 나온다.
